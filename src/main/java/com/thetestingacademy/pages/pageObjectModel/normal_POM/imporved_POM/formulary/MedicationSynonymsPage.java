...
/**
 * Page Object for the Medication Synonyms List page.
 * URL: /medications-order/synonyms
 *
 * Live DOM inspection (2026-07-20) reveals:
 * - Header: TITLE-DIV with exact text "Medication Synonyms List"
 * - Data table: p-table (1 instance)
 * - Buttons: Clear, Ingredient, Add, Upload, Show Tracking, Audit Log, Export
 * - Pagination: pages 1-3
 */
public class MedicationSynonymsPage extends CommonToAllPage {

    @Step("Navigate to Medication Synonyms page and verify it loaded")
    public boolean isMedicationSynonymsPageLoaded() {
        WaitHelpers.checkVisibility(getDriver(), By.cssSelector("ul.layout-menu"), 15);
        getDriver().navigate().to("https://qc.care-connect.health/medications-order/synonyms");
        ...
        By pageHeader = By.xpath("//*[normalize-space(text())='Medication Synonyms List']");
        WaitHelpers.checkVisibility(getDriver(), pageHeader, 15);

        return getDriver().getCurrentUrl().contains("/medications-order/synonyms");
    }
}
