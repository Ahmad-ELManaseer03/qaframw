...
public class DrugDuplicatePage extends CommonToAllPage {

    private final By linkLoc = By.xpath("//a[@href='/cdss-management/drugDuplicate']");
    private final By headerLoc = By.xpath("//*[contains(text(), 'Drug Duplicate List')]");

    public DrugDuplicatePage navigateToDrugDuplicate() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("Could not click Drug Duplicate: " + ex.getMessage());
                try {
                    WebElement link = getDriver().findElement(linkLoc);
                    ((JavascriptExecutor) getDriver()).executeScript("arguments[0].click();", link);
                } catch (Exception e2) {}
    ...
    public boolean isPageLoaded() {
        ...
        return currentUrl.contains("/cdss-management/drugDuplicate");
    }
}
