...
public class PendingOverrideRulesListPage extends CommonToAllPage {

    // Locators
    private final By linkLoc = By.xpath("//a[@href='/rules-management/rules-override/workflows/pending']");
    // Verify using page content (like a table header) rather than just the generic body
    private final By headerLoc = By.xpath("//h5[contains(., 'Pending Override Rules List')] | //div[contains(@class, 'header')]//span[contains(text(), 'Pending Override Rules List')] | //th[contains(., 'Rule ID')]");

    public PendingOverrideRulesListPage navigateToPendingOverrideRulesList() {
        ...
        try {
            ...
            try {
                WebElement menu = getDriver().findElement(By.xpath("//a[.//span[normalize-space(text())='Override Rules Management']]"));
                ...
                System.out.println("Could not click Pending Override Rules List: " + ex.getMessage());
            }
        }
    ...
    public boolean isPageLoaded() {
        ...
        return currentUrl.contains("/rules-management/rules-override/workflows/pending");
    }
}
