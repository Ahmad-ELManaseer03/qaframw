...
public class PrescribingManualOverrideListPage extends CommonToAllPage {

    // Locators
    private final By linkLoc = By.xpath("//a[@href='/rules-management/rules-override/authorization-manual/prescribe']");
    // Verify using page content (like a table header) rather than just the generic body
    private final By headerLoc = By.xpath("//h5[contains(., 'Prescribing Manual Override List')] | //div[contains(@class, 'header')]//span[contains(text(), 'Prescribing Manual Override List')] | //th[contains(., 'Prescribe Reference Number')]");

    public PrescribingManualOverrideListPage navigateToPrescribingManualOverrideList() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("Could not click Prescribing Manual Override List: " + ex.getMessage());
            }
        }
    ...
    public boolean isPageLoaded() {
        ...
        return currentUrl.contains("/rules-management/rules-override/authorization-manual/prescribe");
    }
}
