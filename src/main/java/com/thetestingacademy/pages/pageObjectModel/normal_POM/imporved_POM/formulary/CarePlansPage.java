...
/**
 * Page Object for the Care Plans List page.
 * URL: /medications-order/careplans
 *
 * Live DOM inspection (2026-07-20) reveals:
 * - Header: TITLE-DIV with exact text "Care Plans List"
 * - Data table: p-table (1 instance)
 * - Buttons: Clear, Add, Show Tracking, Audit Log, Export
 * - Pagination: page 1 only
 */
public class CarePlansPage extends CommonToAllPage {

    @Step("Navigate to Care Plans page and verify it loaded")
    public boolean isCarePlansPageLoaded() {
        WaitHelpers.checkVisibility(getDriver(), By.cssSelector("ul.layout-menu"), 15);
        getDriver().navigate().to("https://qc.care-connect.health/medications-order/careplans");
        ...
        By pageHeader = By.xpath("//*[normalize-space(text())='Care Plans List']");
        WaitHelpers.checkVisibility(getDriver(), pageHeader, 15);

        return getDriver().getCurrentUrl().contains("/medications-order/careplans");
    }
}
