...
public class PendingBusinessRulesListPage extends CommonToAllPage {

    // Locators
    private final By linkLoc = By.xpath("//a[@href='/rules-management/rules/workflows/pending']");
    // Verify using page content (like a table header) rather than just the generic body
    private final By headerLoc = By.xpath("//h5[contains(., 'Pending Business Rules List')] | //div[contains(@class, 'header')]//span[contains(text(), 'Pending Business Rules List')] | //th[contains(., 'Rule ID')]");

    public PendingBusinessRulesListPage navigateToPendingBusinessRulesList() {
        ...
        try {
            ...
            try {
                ...
                System.out.println("Could not click Pending Business Rules List: " + ex.getMessage());
            }
        }
    ...
    public boolean isPageLoaded() {
        ...
        return currentUrl.contains("/rules-management/rules/workflows/pending");
    }
}
